			...
			switch( $menu->getType() ) {
				...
				case 'url':
					if(($menu->getUrl() == "" || $menu->getUrl() == "#") && $this->getUrl('') == $this->getUrl('*/*/*', array('_current'=>true, '_use_rewrite'=>true))) {
					    ...
					    $path = str_replace(Mage::getBaseUrl(), '', $currentUrl);

					    $path_routename = $this->getRequest()->getRouteName();

					    $menu_url = $menu->getUrl();

					    $arr_paths = array($menu_url, "/".$menu_url, "/".$menu_url."/","/index.php/".$menu_url."/","/index.php/".$menu_url,"index.php/".$menu_url."/", "index.php/".$menu_url);

					    if(in_array($path, $arr_paths) || in_array($path_routename, $arr_paths)) {
					       $active_class = $this->_menu_item_active_class;
					    }
					}
				break;
